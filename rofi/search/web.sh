...
declare -A URLS
roficonfig=$HOME/.config/rofi/search/config.rasi
URLS=(
...
manolo(){
    word=$( (echo ) | rofi  -dmenu -config $roficonfig -matching fuzzy -location 0 -p "Query > " )
    [ -z "$word" ] && exit
    serach="https://www.google.com/search?q="$word
    /usr/bin/surf "$serach"
    # google-chrome-stable "$serach"
}
manolo
# main
